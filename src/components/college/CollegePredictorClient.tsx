...
type PredictorStep = 'account' | 'payment' | 'predict' | 'results';
...
const STEP_SEQUENCE: Array<{ id: PredictorStep; title: string; caption: string }> = [
  { id: 'account', title: 'Account', caption: 'Login or register' },
  { id: 'payment', title: 'Access', caption: 'Pay and verify' },
  { id: 'predict', title: 'Predict', caption: 'Enter rank details' },
  { id: 'results', title: 'Results', caption: 'Review options' },
];

function JourneyStepper({ currentStep }: Readonly<{ currentStep: PredictorStep }>) {
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-extrabold ${isCurrent ? 'text-orange' : 'text-gray-900'}`}>
                    {step.title}
                  </p>
                  <p className="text-xs text-gray-500">{step.caption}</p>
                </div>
              </div>
            </div>
          );
...
function AccessPanel({
  ...
  return (
    <aside className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-lg shadow-gray-200/70">
      <div className="border-b border-gray-100 bg-gray-50 px-5 py-4">
        <p className="text-xs font-bold uppercase tracking-wide text-orange">
          {user ? (hasAccess ? 'Access active' : 'Payment verification') : 'Student account'}
        </p>
        <h2 className="mt-1 text-lg font-extrabold text-gray-900">
          {user ? (hasAccess ? 'You are ready to predict' : 'Unlock 7-day access') : 'Start with login'}
        </h2>
      </div>
...
export function CollegePredictorClient() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FB]">
      <section className="bg-[#0D1B3E] pb-16 pt-10 text-white sm:pb-20 sm:pt-14">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1fr_340px] lg:items-end">
            <div className="max-w-3xl">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange/40 bg-white/10 px-4 py-1.5">
                <span className="h-2 w-2 rounded-full bg-orange" />
                <span className="text-sm font-semibold text-orange">NEET MBBS Predictor 2025</span>
              </div>
              <h1 className="font-heading text-3xl font-bold leading-tight sm:text-4xl lg:text-5xl">
                Find realistic MBBS options from your NEET rank.
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-gray-300 sm:text-lg">
                A protected 7-day predictor flow with backend-managed cutoff data, category split, quota filters, and
                verification before results unlock.
              </p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 p-5 backdrop-blur">
              <p className="text-sm font-semibold text-gray-300">Access plan</p>
              <div className="mt-2 flex items-end justify-between gap-4">
                <div>
                  <p className="text-3xl font-extrabold text-white">{formatMoney(plan?.amountPaise || 58882)}</p>
                  <p className="mt-1 text-xs text-gray-300">Includes GST</p>
                </div>
                <span className="rounded-full bg-orange px-3 py-1 text-xs font-extrabold text-white">
                  {plan?.accessDays || 7} days
                </span>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-xl bg-white/10 p-3">
                  <p className="text-gray-300">Data source</p>
                  <p className="mt-1 font-bold text-white">Backend API</p>
                </div>
                <div className="rounded-xl bg-white/10 p-3">
                  <p className="text-gray-300">Payment</p>
                  <p className="mt-1 font-bold text-white">
                    {plan?.isPaymentConfigured ? 'Online' : 'Manual'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-lg shadow-gray-200/70">
            <div className="border-b border-gray-100 bg-white px-6 py-5 sm:px-8">
              ...
              <div className="mt-1 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h2 className="text-xl font-extrabold text-gray-900">Enter your rank and counselling filters</h2>
                  <p className="mt-1 text-sm text-gray-500">
                    {hasAccess ? 'Access is active. Your search is ready.' : 'Complete account and access steps to unlock search.'}
                  </p>
                </div>
                <span
                  className={`inline-flex w-fit rounded-full px-3 py-1 text-xs font-bold ${
                    hasAccess ? 'bg-green-50 text-green-700 ring-1 ring-green-200' : 'bg-gray-100 text-gray-500'
                  }`}
                >
      ...
      {!response && (
        <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              { title: 'Protected search', desc: 'Results unlock only after student access is active.' },
              { title: 'Manual verification', desc: 'Transaction details can be submitted for admin approval.' },
              { title: 'Backend data', desc: 'Cutoff metadata and results come from the live API.' },
            ].map((card) => (
