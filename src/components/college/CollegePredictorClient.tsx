...
type PredictorResultsSectionProps = {
...
type PredictorFormSubmitEvent = Parameters<NonNullable<React.ComponentProps<'form'>['onSubmit']>>[0];

function PredictorResultsSection({ response, resultsRef, page, setPage }: Readonly<PredictorResultsSectionProps>) {
  const paginatedResults = response.results.slice(0, page * ITEMS_PER_PAGE);
  const hasMore = paginatedResults.length < response.results.length;
  const collegeSuffix = response.totalMatches === 1 ? '' : 's';
  const resultsHeading = response.totalMatches > 0
    ? `${response.totalMatches.toLocaleString()} College${collegeSuffix} Found`
    : 'No Colleges Found';
  ...
  return (
    <section ref={resultsRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">{resultsHeading}</h2>
      ...
      {response.totalMatches === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
          ...
          <Link href="/contact" className="inline-flex items-center gap-2 h-10 px-6 rounded-full bg-orange text-white text-sm font-bold hover:bg-orange-hover transition-colors">
        ...
        <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#0D1B3E] text-white">
                    ...
                    <th className="text-left px-4 py-3 font-semibold min-w-70">College Name</th>
                    <th className="text-left px-4 py-3 font-semibold min-w-32.5">State</th>
                    <th className="text-left px-4 py-3 font-semibold min-w-25">Category</th>
                    <th className="text-left px-4 py-3 font-semibold min-w-27.5">Sub Category</th>
                    <th className="text-left px-4 py-3 font-semibold min-w-27.5">Closing Rank</th>
                    <th className="text-left px-4 py-3 font-semibold min-w-32.5">Quota</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedResults.map((result, index) => {
                    const rowKey = `${result.state}-${result.college}-${result.rawCategory}-${result.closingRank}-${result.quota}`;
                    const rowNumber = index + 1;
                    const rowTone = index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50';

                    return (
                      <tr key={rowKey} className={`border-t border-gray-100 ${rowTone} hover:bg-orange-50/30 transition-colors`}>
                        <td className="px-4 py-3 text-gray-400 text-xs">{rowNumber}</td>
          ...
          <div className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-900">
            ...
            <Link href="/contact" className="inline-block mt-2 text-orange font-semibold hover:underline">Book free counselling →</Link>
          </div>
        </>
      )}
    </section>
...
function PredictorHighlightsSection() {
  return (
...
export function CollegePredictorClient() {
  ...
  const hasActiveFilters = Boolean(state || category || subCategory || quota);
  const showIdleState = response === null && !loading;

  const handlePredict = useCallback(async (event: PredictorFormSubmitEvent) => {
    event.preventDefault();
    const rankNum = Number.parseInt(rank, 10);
    if (Number.isNaN(rankNum) || rankNum <= 0 || rankNum > 2000000) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <section className="bg-[#0D1B3E] py-12 sm:py-16 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-orange/40 bg-orange/20 px-4 py-1.5 mb-4">
            ...
            <span className="text-orange text-sm font-semibold">NEET UG 2025 · 21 States · Final Cutoffs</span>
          </div>
          ...
          <form onSubmit={handlePredict} className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full h-12 px-4 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange/50 focus:border-orange text-gray-900 text-base transition-colors"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full h-12 px-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange/50 focus:border-orange text-gray-900 text-sm bg-white appearance-none cursor-pointer disabled:opacity-50"
                >
              ...
              <div>
                <label htmlFor="predictor-category" className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Category
                  {state && (
                    <span className="ml-1 text-xs text-orange font-normal">({stateCategories.length})</span>
                  )}
                </label>
                <select
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  ...
                  {state && stateQuotas.length > 1 && (
                    <span className="ml-1 text-xs text-orange font-normal">({stateQuotas.length})</span>
                  )}
                </label>
                <select
                  value={quota}
            ...
            {showSubCategoryControl && (
              <div>
                <label htmlFor="predictor-sub-category" className="block text-sm font-semibold text-gray-700 mb-1.5">
                  ...
                  <span className="ml-1 text-xs text-orange font-normal">
      ...
      {response && <PredictorResultsSection response={response} resultsRef={resultsRef} page={page} setPage={setPage} />}

      {showIdleState && <PredictorHighlightsSection />}
    </div>
  );
}